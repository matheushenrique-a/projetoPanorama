...
class Indicadores extends BaseController
{
    ...
    public function indicadores_diarios(){
        ...
        $propostasPagas30 = $this->dbMaster->runQuery("select p.statusProposta, count(p.statusProposta) numero, sum(valor_pago) valor from proposta_fgts p inner join proposta_fgts_gravacao_json g on p.id_proposta = g.id_proposta where p.statusProposta = 'PASSO 09 - PROPOSTA FINALIZADA' group by p.statusProposta;");
